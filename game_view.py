...
class Switch(TileData):
    def __init__(self, map, type, pos):
        ...
        doors.sort(key=lambda x: (x.pos - self.pos).SquareLength())
        self.door = doors[0]
...
class Computer(TileData):
    ...
    def SetScreen(self, parent):
        ...
        if self.terminal is None:
            self.terminal = self.terminal_type(
                parent=self.screen,
...
terminals = {
    TileTypes.PIN_ENTRY: terminal.GrotoEntryTerminal,
...
class GameMap(object):
    ...
    def __init__(self, name):
        ...
        self.data = [[TileTypes.GRASS for i in range(self.size.y)] for j in range(self.size.x)]
        self.actors = []
        self.doors = []
        self.computers = []
        self.switches = []
...
class GameView(ui.RootElement):
    def __init__(self):
        ...
        self.text.box = ui.Box(
            parent=self.text, pos=Point(-0.1, -0.2), tr=Point(1.1, 1.2), colour=(0, 0, 0, 0.3)
        )
        self.switch_text = ui.TextBox(
            globals.screen_root,
            bl=Point(0.15, 0.15),
            tr=None,
        ...
        self.switch_text.box = ui.Box(
            parent=self.switch_text, pos=Point(-0.1, -0.2), tr=Point(1.1, 1.2), colour=(0, 0, 0, 0.3)
        )
        self.actor_text = ui.TextBox(
            globals.screen_root,
            bl=Point(0.15, 0.15),
            tr=None,
        ...
        self.actor_text.box = ui.Box(
            parent=self.actor_text, pos=Point(-0.1, -0.2), tr=Point(1.1, 1.2), colour=(0, 0, 0, 0.3)
        )
        self.text.Disable()
    ...
    def StartMusic(self):
        ...
        self.music_text.box = ui.Box(
            parent=self.music_text, pos=Point(-0.1, -0.2), tr=Point(1.1, 1.2), colour=(0, 0, 0, 0.9)
        )
